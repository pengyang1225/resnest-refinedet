...
class RefineDetMultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        for idx in range(num):
            ...
            if((num_classes == 2) & (self.use_ARM == 0)):
                labels = labels >= 0
        ...
        batch_conf = conf_data.view(-1, self.num_classes).contiguous()
        ####################bug place
        AA = log_sum_exp(batch_conf).contiguous()
        BB = batch_conf.gather(1, conf_t.view(-1, 1)).contiguous()
        loss_c = AA - BB
        #print(loss_c.size())
